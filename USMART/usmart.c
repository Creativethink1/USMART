...
{
	u8 sta;  
	if((usmart_dev.Rxflag&0x80)==0x80)//串口接收完成
	{					   
		if(usmart_dev.Rxnum!=0)//数据个数不为零
		{
			usmart_dev.Rxbuff[usmart_dev.Rxnum]='\0';	//在末尾加入结束符. 
			sta=usmart_dev.cmd_rec(usmart_dev.Rxbuff);//得到函数各个信息
...
{
u8 res=chr;
	if((usmart_dev.Rxflag&0x80)==0)//接受未完成
	{
		if((usmart_dev.Rxflag&0x40)==0x40)//接受到‘\r’
		{
			if(res=='\n')
			{
			usmart_dev.Rxflag|=0x80;//接受完成
			}
